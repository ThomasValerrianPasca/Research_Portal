...
function sp_find_icon($path, $file) {
	...
	# find icon checking theme, parent and finally path
    $curTheme = sp_get_current_sp_theme();
	if (file_exists(SPTHEMEICONSDIR.$file)) {
		return SPTHEMEICONSURL.$file;
	} else if (!empty($curTheme['parent']) && file_exists(SPTHEMEBASEDIR.$curTheme['parent'].'/images/'.$file)) {
		return SPTHEMEBASEURL.$curTheme['parent'].'/images/'.$file;
	} else {
		return $path.$file;
	}
}
