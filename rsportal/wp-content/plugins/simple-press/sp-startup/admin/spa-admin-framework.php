...
Simple:Press
Admin Panels
$LastChangedDate: 2014-05-04 17:21:56 -0700 (Sun, 04 May 2014) $
$Rev: 11385 $
*/
...
function spa_check_warnings() {
	...
	if (empty($cur)) {
		$mess.= spa_message(spa_text('No main theme has been selected and SP will be unable to display correctly. Please select a theme from the Themes panel'));
	} else {
    ...
    if (!empty($mobile) && $mobile['active']) {
		...
		if ($nostylesheet || ($nooverlay && $nopoverlay)) {
			$mess.= spa_message(spa_text('Either the mobile theme CSS file and/or color Overlay file from the selected mobile theme is missing'));
		}
	}
    ...
    if (!empty($tablet) && $tablet['active']) {
		...
		if ($nostylesheet || ($nooverlay && $nopoverlay)) {
			$mess.= spa_message(spa_text('Either the tablet theme CSS file and/or color Overlay file from the selected tablet theme is missing'));
		}
	}
