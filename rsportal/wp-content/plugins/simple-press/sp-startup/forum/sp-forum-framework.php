...
Simple:Press
Desc:
$LastChangedDate: 2014-05-24 01:12:47 -0700 (Sat, 24 May 2014) $
$Rev: 11461 $
*/
...
function sp_load_plugin_styles() {
	...
	if (sp_is_plugin_active('user-selection/sp-user-selection-plugin.php')) {
		if (!empty($parentTheme)) wp_enqueue_style('sp-parent', SPTHEMEBASEURL.$curTheme['parent'].'/styles/'.$parentTheme['Stylesheet'].$color);
		wp_enqueue_style('sp-theme', SPTHEMECSS.$color);
	} else {
		if (!empty($parentTheme)) sp_plugin_enqueue_style('sp-parent', SPTHEMEBASEURL.$curTheme['parent'].'/styles/'.$parentTheme['Stylesheet'].$color);
		sp_plugin_enqueue_style('sp-theme', SPTHEMECSS.$color);
	}
...
function sp_forum_footer() {
	global $spVars, $spGlobals, $spThisUser, $spMobile;
