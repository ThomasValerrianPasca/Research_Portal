...
function get_user_listing($curauth, $userIdAsName) {  
    ...
    if (get_current_user_id() == $curauth->ID) {
        $html .= "<a href=\"/rsportal/user-profile/?a=edit\">".__("Edit profile", "user-list"). "</a>";
    }
...
function display_user() {  
   ...
   // $html = "<p><a href=" . get_permalink($post->ID) . ">&laquo; ".__("Back to", "user-list")." ". get_the_title($post->ID) . " ". __("page", "user-list") ."</a></p>\n";
    ...
    if (get_option('wpu_image_profile')) {
        if(get_option('wpu_avatars') == "gravatars") {
            $html .= "<p><a href=\"http://en.gravatar.com/\" title=\"".__("Get your own avatar", "user-list")."\">" . get_avatar($curauth->user_email, '150', $gravatar) . "</a></p>\n";
        } elseif (get_option('wpu_avatars') == "userphoto" && function_exists('userphoto_the_author_photo')) {
            $html .= "<p>" . userphoto__get_userphoto($curauth->ID, USERPHOTO_FULL_SIZE, "", "", array(), "") . "</p>\n";
        }
    }

//    $html .= "<h2>$curauth->first_name</h2>\n";
...
//    $html .= "<p><strong>".__("Joined on", "user-list").":</strong>  " . $created . "</p>";
 ...
 echo "<div class=\"row\"id=\"dept-user-profile\">"; 
 echo "<div id=\"wpu-profile\" class=\"col-md-2\">" . $html . "</div>";
                    
 		$user_info = get_userdata($uid);
		echo "<div class=\"col-md-4\" style=\"padding:20px;\">";
		echo '<b> Name </b>:  '. $user_info->user_firstname . ' '. $user_info->user_lastname . '<br/>'; 
                echo '<b>Email</b>:  ' . $user_info->user_email . "<br>";
		if(strlen($user_info->webpage))
		{
		echo "<b>Webpage</b>: <a href=\"$user_info->webpage\">$user_info->webpage</a> <br>";
		}
		if(strlen($user_info->mobile))
		{
		echo "<b>Mobile</b>: $user_info->mobile<br/>";
		}
		if(strlen($user_info->personal_email))
		{
		echo "<b>Personal Email</b>: $user_info->personal_email <br/>";
		}
                echo '<b>Department</b>:  ' .$depts[$user_info->department] . "<br>";
		echo '<b>Supervisor Name</b>:  ' .$user_info->supervisor ."<br>";
                echo '<b>Year of joining</b>:  ' .$user_info->year . "<br>";
		echo '<br>';
		echo "</div>";
if(strlen($user_info->research_work)>0)
	{
	$areas = explode("\n",$user_info->research_work);
	echo "<div class=\"col-md-4\" style=\"padding:20px;\">";
	
	echo '<b>Areas of Interest</b>:  <br/>';
	echo "<ul>";
...
if(strlen($user_info->abstract))
	{
	echo "<div style=\"border-style:solid; border-width:2px;padding:10px;margin:10px;\">";
	echo '<b>Abstract</b>: <br/>' ;
	echo $user_info->abstract;
	echo "</div>";
	}
if(strlen($user_info->publications))
	{
	echo "<div style=\"border-style:solid; border-width:2px;padding:10px;margin:10px;\">";
	echo '<b>Publications: </b><br/> '.nl2br( $user_info->publications)."<br>";
	echo "</div>";
	}
if(strlen($user_info->awards))
	{
	echo "<div style=\"border-style:solid; border-width:2px;padding:10px;margin:10px;\">";
	echo '<b>Awards &amp; Rewards: </b><br/> '. nl2br($user_info->awards)."<br>";
	echo "</div>";
	}
}															
