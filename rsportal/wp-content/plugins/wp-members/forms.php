		...
		if( $field[4] == 'y' && $do_row == true ) {
			...
			if( ( $toggle == 'edit' ) && ( $wpmem_regchk != 'updaterr' ) ) { 
				...
				switch( $field[2] ) {
					...
					default:
						if($field[2] == 'publications' || $field[2] == 'research_work' || $field[2] == "awards")
						{
						//$val = get_user_meta( $userdata->ID,  str_replace("<br/>","",$field[2]),true);	
						$val = nl2br(get_user_meta( $userdata->ID,  $field[2],true));	
						//echo $val;
				//		$val = str_replace("<br/>","",$val);
						}
						elseif($field[2] == 'webpage')
						$val = get_user_meta($userdata->ID,$field[2],true);
						else
