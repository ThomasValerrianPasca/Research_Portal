...
function wpmem_registration( $toggle )
{
	...
	foreach( $wpmem_fields as $meta ) {
		if( $meta[4] == 'y' ) 
		{
			if( $meta[2] != 'password'  && $meta[2] != 'publications' && $meta[2] != 'research_work') 
			{
			$fields[$meta[2]] = ( isset( $_POST[$meta[2]] ) ) ? sanitize_text_field( $_POST[$meta[2]] ) : '';
			} 
			elseif($meta[2] == 'publications' || $meta[2] == 'research_work' || $meta[2] == 'awards') 
			{
			$fields[$meta[2]] = ( isset( $_POST[$meta[2]] ) ? str_replace("<br />","",$_POST[$meta[2]]) : '');
			}
			else 
			{
...
do_action( 'wpmem_update_redirect' );

		// _post_update_data hook is after insertion
//		do_action( 'wpmem_post_update_data', $fields );
//		do_action( 'wpmem_post_update_data', 'updateredirect', 1 );
